...
function parseResponse(response: string): GeneratedContent {
  ...
  for (const line of lines) {
    ...
    const keyMatch = line.match(/^(SHORT_ONE_LINER|DESCRIPTION|BEST_FOR_\d|NOT_FOR_\d|FEATURE_\d_TITLE|FEATURE_\d_DETAIL):\s*(.*)/)
  ...
  const shortOneLiner = fields['SHORT_ONE_LINER'] || ''
